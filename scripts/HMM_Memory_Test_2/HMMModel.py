...
class HMMModel(object):
    ...
    #training part
    def trainingHMM(self, trainfile, n1, n3):
        f = open(trainfile)
        try:
            reader = C.reader(f)
            floats = []
    ...
    def testingHMM(self, filename, n, p, model, dataArray):
        scorelist = []
        f = open(filename)
        try:
            reader = C.reader(f)
    ...
    def drawROC(self, scorelist, labellist, size):
        ...
        average = sum(scorelist)/float(len(scorelist))
        min_len = average + average*50/float(100)
